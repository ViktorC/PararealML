...
from typing import Sequence, List, Union, Tuple, Callable
...
class Mesh:
    ...
    def _create_boundary_constraint_functions(self) \
        ...
        """
        def set_boundary_and_mask_values(_bc_condition_func, _boundary, _mask):
            _mask[slicer] = 1
        ...
        for fixed_axis in range(len(self._y_shape) - 1):
            ...
            if lower_bc.has_y_condition():
                set_boundary_and_mask_values(
                    ...
                    constrained_y_values[slicer],
                    y_mask)
            if lower_bc.has_d_y_condition():
                set_boundary_and_mask_values(
                    lower_bc.d_y_condition,
                    constrained_d_y_values[slicer],
                    d_y_mask)
    ...
    def d_x(self) -> Sequence[float]:
        """
        Returns the step sizes along the spatial dimensions
        """
    ...
    def d_y_constraint_func(self) -> Callable[[np.ndarray], None]:
        ...
        derivative of y evaluated on the mesh.
        """
