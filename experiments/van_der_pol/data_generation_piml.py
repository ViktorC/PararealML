...
def generate_initial_conditions() -> np.ndarray:
    ...
    initial_conditions = np.empty((5000,) + ivp.constrained_problem.y_shape())
    n_sub_ivps = 4
    for i in range(n_sub_ivps):
        ...
        for j in range(len(initial_conditions) // n_sub_ivps):
            ...
            ] = base_y_0 + np.random.normal(0.0, 0.2, base_y_0.shape)
...
if __name__ == "__main__":
    ...
    from experiments.van_der_pol.operators import coarse_fdm
